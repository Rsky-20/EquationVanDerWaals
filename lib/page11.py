...
def ConstuctionMaxwell(master):
    ...
    T_r=np.arange(130, 180, 10)
    V_r=np.arange(0, 0.0005, 0.0001)
    
    #palette = iter(['#9b59b6', '#4c72b0', '#55a868', '#c44e52', '#dbc256', '#94331E', '#1E9484', '#25941E','#948F1E', '#1E3994', '#941E44'])
    ...
    def plot_pV(T):
        Tr = T / Tc
        ax.plot(Vr, vdw(Tr, Vr), lw=2, alpha=0.3, color='r')
        ax.plot(Vr, vdw_maxwell(Tr, Vr), lw=2, color='r', label='{:.2f}'.format(Tr))
    ...
    ax.axis([0.4, 4, 0, 2])
    ax.set_title("Isotherme d'un gaz de VDW")
